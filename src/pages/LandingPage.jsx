...
import { ArrowRight, Shuffle, BarChart2, FileText, Clock, Maximize2, History, MessageSquare, TrendingUp, ChevronDown, Check } from "lucide-react";
...
const features = [
  ...
  { icon: Shuffle, tag: "MEETING", title: "Speaking Order", desc: "Randomize who goes first every time. Fair, instant, no awkward silence." },
];

const faqs = [
  { q: "How is Quorum different from Geekbot or Standuply?", a: "Most standup tools are async message bots. Quorum is built for live weekly standups — with present mode, time tracking, and AI that understands your team's full history. The AI Analyst is something no other standup tool offers." },
  { q: "Do I need to set up any integrations?", a: "No. Sign up, create a team, share the invite code. Your team can be running in under 5 minutes. No Slack bots, no calendar integrations, nothing to configure." },
  { q: "What does the AI actually do?", a: "Two things. The Executive Summary pulls your team's real weekly data and writes a polished summary you can send to leadership — refineable in a chat interface. The AI Analyst lets you ask free-form questions about trends across up to 52 weeks of history for your whole team." },
  { q: "Is our data secure?", a: "Yes. Authentication is handled by Clerk (SOC 2 certified). Data is stored in Supabase (SOC 2 certified). All traffic is encrypted via HTTPS. We don't sell your data to anyone." },
  { q: "How much does it cost?", a: "Quorum is free to get started. No credit card required." },
  { q: "Can I use Google to sign in?", a: "Yes — Google sign-in is fully supported alongside email and password. Most teams use it since everyone already has a Google account." },
];
  ...
  return (
    <div style={{ minHeight: "100vh", background: P.white, fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      ...
      <div style={{ padding: "0 24px 96px", maxWidth: 760, margin: "0 auto" }}>
        <p style={{ fontSize: 11, fontWeight: 800, color: P.red, letterSpacing: 3, textAlign: "center", marginBottom: 14 }}>FAQ</p>
        <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(28px, 4vw, 44px)", textAlign: "center", color: P.gray700, marginBottom: 56, fontWeight: 400, letterSpacing: -0.5 }}>Questions answered</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {faqs.map((f, i) => (
            <details key={i} className="faq-item" style={{ background: P.white, border: `1.5px solid ${P.gray100}`, borderRadius: 16, overflow: "hidden" }}>
              <summary style={{ padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, userSelect: "none" }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: P.gray700 }}>{f.q}</span>
                <ChevronDown size={18} color={P.gray400} style={{ flexShrink: 0, transition: "transform 0.2s" }} />
              </summary>
              <div style={{ padding: "16px 24px 20px", fontSize: 14, color: P.gray500, lineHeight: 1.75, borderTop: `1px solid ${P.gray100}` }}>{f.a}</div>
            </details>
          ))}
        </div>
      </div>
      ...
      <div style={{ borderTop: `1px solid ${P.gray100}`, padding: "32px 32px" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          <Logo size={26} />
          <p style={{ fontSize: 12, color: P.gray400, fontWeight: 500 }}>© 2026 Quorum · Before, during, and after.</p>
